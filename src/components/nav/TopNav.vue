<template>
	<div class="nav-wrap" :style="nav_class_top ? 'height:'+ (height) + 'px': ''">
		<nav class="nav"
...
<script setup lang="ts">
	...
	const onScroll = (e: Event) => {
		...
		const scr = <Document>e.target
			scroll.value=scr.documentElement.scrollTop
		if ( scr.documentElement.scrollTop > pos.value)
			nav_class_top.value = true
		else
			nav_class_top.value = false
	}
